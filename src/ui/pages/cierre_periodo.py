...
def show():
    ...
    st.caption("Equivalente a actualiza.frm del macro EPSACOL (actua2 + actua3)")
    ...
    st.markdown("""
    Equivalente a **actua2_Click** del macro:
    - Copia lecturas del buffer → hoja base (en la app: verifica tabla `Lectura`)
    - Calcula consumo e importe por rangos de tarifa
    - Muestra deuda acumulada y total a pagar (columnas K, L, N de la hoja BASE)
    """)
    ...
    st.markdown("**Vista previa estilo Hoja BASE:**")
    ...
    st.markdown("""
    Equivalente a **actua3_Click** del macro:
    - Pasa lectura actual → lectura anterior (automático en la próxima toma)
    - Genera registros en tabla **Deuda** para los que NO pagaron
