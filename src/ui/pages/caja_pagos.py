...
def show():
    ...
    st.caption("Equivalente a Cliente.frm + Deudas.frm del macro EPSACOL")
    ...
    st.markdown("Escriba código o nombre y presione **Buscar**")

    col_bus1, col_bus2 = st.columns([4, 1])
    with col_bus1:
        search_term = st.text_input("Código o nombre", placeholder="Ej: AP-001 o GONZALES", key="caja_search_term")
    with col_bus2:
        btn_buscar = st.button("🔍 Buscar", use_container_width=True)

    if btn_buscar and search_term:
    ...
    if "caja_resultados_busqueda" in st.session_state:
        ...
        if not resultados:
            ...
            sel = st.selectbox("Seleccione usuario (simula doble clic en lista)", list(opciones.keys()), key="caja_sel_usuario")
        ...
        if st.button("✅ REGISTRAR PAGO E IMPRIMIR RECIBO", type="primary", use_container_width=True):
            ...
            try:
                with get_session() as session:
                    ...
                    nro_recibo = f"REC-{date.today().strftime('%Y%m%d')}-{max_id + 1}"

                    # 1) OBTENER DEUDAS FRESAS DE LA BD (no usar las de la pantalla anterior)
                    ...
                    for deuda in deudas_db:
                        if monto_restante <= 0.01:  # Tolerancia
                            break
                    ...
                    pago_total = Pago(
                        ...
                        monto=efectivo_redondeado,  # Registramos lo que realmente pagó
                    ...
                    datos_recibo = {
                        ...
                        'importe_consumo': importe_periodo,
                    ...
                    # NOTA: NO tocamos caja_search_term para evitar error de Streamlit
                    st.session_state.caja_mensaje = {
                        "tipo": "success",
                        "texto": f"✅ Pago registrado. Recibo {nro_recibo}. Cambio: {cambio:.2f} Bs",
                        "pdf_ruta": ruta_pdf,
                    ...
                    # st.session_state.caja_search_term = ""  # ❌ ELIMINADO: causa error en Streamlit
