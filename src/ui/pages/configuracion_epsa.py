...
def show():
    ...
    st.caption("Gestión de Empresas Prestado ras de Servicios de Agua")
    ...
    with tab_lista:
        ...
        if not epsas:
            ...
            data = []
            for e in epsas:
                # Datos primitivos, NO objetos ORM fuera de sesión
                with get_session() as session:
                    config = session.exec(
                        select(ConfiguracionEPSA).where(ConfiguracionEPSA.epsa_id == e.id)
                    ).first()
                    periodo = session.exec(
                        select(Periodo).where(Periodo.epsa_id == e.id, Periodo.activo == True)
                    ).first()
            ...
            opciones = {f"{e.nombre} ({e.ciudad})": e.id for e in epsas}  # Guardar IDs, no objetos
            ...
            with col1:
                if st.button("✅ Activar selección", type="primary"):
                    ...
                    st.success(f"EPSA activa actualizada.")
                    st.rerun()
            with col2:
                if st.session_state.get("epsa_id"):
                    # Buscar nombre de la activa en la lista ya cargada
    ...
    with tab_nueva:
        ...
        with st.form("form_nueva_epsa"):
            ...
            submitted = st.form_submit_button("➕ Crear EPSA", type="primary", use_container_width=True)
                ...
                if not nombre or not ciudad:
                    ...
                    try:
                        with get_session() as session:
                            ...
                            nueva_epsa_id = nueva_epsa.id  # Guardar ID primitivo
                        ...
                        # Activar automáticamente (fuera de la sesión)
    ...
    with tab_config:
        ...
        with get_session() as session:
            ...
            epsa_nombre = epsa.nombre  # Extraer string, no objeto
            ...
            # Extraer todos los valores primitivos ANTES de cerrar la sesión
